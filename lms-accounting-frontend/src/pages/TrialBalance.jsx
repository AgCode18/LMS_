      ...
      <PageHeader
        ...
        action={
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            ...
            />
            <button type="button" className={buttonClass} onClick={() => load({ asOf: asOf || undefined })}>
              Run
            </button>
          </div>
        }
        ...
        <>
          <div className="grid gap-4 md:grid-cols-3">
            <div className={statCardClass}>
              ...
              <div className="mt-3 text-3xl font-semibold text-slate-900">{money(data.totalDebit)}</div>
            </div>
            <div className={statCardClass}>
              <div className="text-sm text-slate-500">Total credit</div>
              <div className="mt-3 text-3xl font-semibold text-slate-900">{money(data.totalCredit)}</div>
            </div>
            <div className={statCardClass}>
          ...
          <div className="mt-6 rounded-4xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="mb-5 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              ...
              <ExportButtons
                onExport={(format) => api.exportTrialBalance(format, { asOf: asOf || undefined })}
              />
            </div>
              ...
              <table className="min-w-full divide-y divide-slate-200 text-sm text-slate-700">
                <thead className="bg-slate-50 text-slate-500">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-right font-semibold">Total debit</th>
                ...
                <tbody className="divide-y divide-slate-200 bg-white">
                  {data.rows.map((r) => (
                    <tr key={r.accountId} className="odd:bg-slate-50">
                      ...
                      <td className="px-4 py-3 font-mono text-slate-600">{r.type}</td>
                      <td className="px-4 py-3 text-right font-medium text-rose-700">{r.totalDebit ? money(r.totalDebit) : ''}</td>
                      <td className="px-4 py-3 text-right font-medium text-emerald-700">{r.totalCredit ? money(r.totalCredit) : ''}</td>
                      <td className="px-4 py-3 text-right font-semibold text-slate-900">{money(r.closingBalance)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
