...
export default function BranchWise() {
  ...
  async function load() {
    ...
    try {
      const result = await api.getBranchWise({
        ...
        branchSearch: branchSearch || undefined,
      });
      setData(result);
    } catch (e) {
      setError(e.message);
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div className="grid gap-4 xl:grid-cols-[1.2fr_1fr_1fr_1fr_auto] items-end">
          <div>
          ...
          <div>
            <label className="block text-sm font-medium text-slate-600">Search branch</label>
            <input
              value={branchSearch}
              onChange={(e) => {
      ...
      <div className="rounded-4xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        {data && data.disbursementsByBranch.length > 0 && (
          <>
                  ...
                  {currentDisbursementRows.map((row) => (
                    <tr key={row.branchId}>
                      <td className="px-4 py-3 font-mono text-slate-700">
                        {row.branchCode ? `${row.branchCode} — ${row.branchName}` : row.branchId}
                      </td>
            ...
            {disbursementRows.length > PAGE_SIZE && (
              <Pagination page={disbursementsPage} pageSize={PAGE_SIZE} total={disbursementRows.length} onPageChange={setDisbursementsPage} />
            )}
          </>
        )}
      </div>
      ...
      <div className="rounded-4xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        {data && data.applicationsByBranch.length > 0 && (
          <>
            ...
            {applicationRows.length > PAGE_SIZE && (
              <Pagination page={applicationsPage} pageSize={PAGE_SIZE} total={applicationRows.length} onPageChange={setApplicationsPage} />
            )}
          </>
        )}
      </div>
