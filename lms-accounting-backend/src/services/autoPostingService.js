...
export async function postLoanDisbursement({
  ...
  if (!resolvedDisbursementId) {
    const loanApp = await prisma.loanApplication.findUnique({ where: { id: loanApplicationId } });
    if (!loanApp) throw new Error(`LoanApplication ${loanApplicationId} not found`);
  ...
  const entry = await createJournal({
    ...
    lines: [
      { accountId: loanReceivable.id, debit: amount, description: 'Loan principal receivable' },
      { accountId: bank.id, credit: amount, description: 'Funds disbursed from bank' },
    ],
  });
...
export async function postEmiCollection({
  ...
  if (total <= 0) throw new Error('EMI collection amount must be greater than zero');

  let resolvedEmiPaymentId = emiPaymentId;
  if (!resolvedEmiPaymentId) {
    if (!emiScheduleId) throw new Error('Provide either emiPaymentId or emiScheduleId');
  ...
  const needed = [KEYS.BANK_ACCOUNT, KEYS.LOAN_RECEIVABLE, KEYS.INTEREST_INCOME];
  if (penaltyAmount > 0) needed.push(KEYS.PENALTY_INCOME);
  if (bounceAmount > 0) needed.push(KEYS.BOUNCE_CHARGE_INCOME);

  const accounts = Object.fromEntries(
    await Promise.all(needed.map(async (key) => [key, await getBySystemKey(key)]))
  );

  const lines = [{ accountId: accounts[KEYS.BANK_ACCOUNT].id, debit: total, description: 'EMI received' }];
  if (principalAmount > 0) {
    lines.push({ accountId: accounts[KEYS.LOAN_RECEIVABLE].id, credit: principalAmount, description: 'Principal component' });
  }
  if (interestAmount > 0) {
    lines.push({ accountId: accounts[KEYS.INTEREST_INCOME].id, credit: interestAmount, description: 'Interest component' });
  }
  if (penaltyAmount > 0) {
    lines.push({ accountId: accounts[KEYS.PENALTY_INCOME].id, credit: penaltyAmount, description: 'Late payment penalty' });
  }
  if (bounceAmount > 0) {
    lines.push({ accountId: accounts[KEYS.BOUNCE_CHARGE_INCOME].id, credit: bounceAmount, description: 'Bounce charges' });
  }
...
// ---------------------------------------------------------------------
export async function postPenaltyCollection({ referenceId, amount, collectionDate, narration }) {
  ...
  return createJournal({
    ...
    lines: [
      ...
      { accountId: penaltyIncome.id, credit: amount, description: 'Penalty income' },
    ],
  });
}
...
// ---------------------------------------------------------------------
export async function postProcessingFee({ loanApplicationId, amount, collectedImmediately = true, feeDate, narration }) {
  const [debitAccount, feeIncome] = await Promise.all([
    getBySystemKey(collectedImmediately ? KEYS.BANK_ACCOUNT : KEYS.CUSTOMER_RECEIVABLE),
    ...
    lines: [
      {
        ...
        debit: amount,
        description: collectedImmediately ? 'Processing fee collected' : 'Processing fee receivable',
      },
      { accountId: feeIncome.id, credit: amount, description: 'Processing fee income' },
    ],
  });
}
...
// ---------------------------------------------------------------------
export async function postRefund({ referenceId, amount, refundDate, reason, narration }) {
  ...
  return createJournal({
    ...
    lines: [
      { accountId: feeIncome.id, debit: amount, description: 'Reversal of income' },
...
// 6. Loan Write-Off
// ---------------------------------------------------------------------
export async function postWriteOff({ loanApplicationId, amount, writeOffDate, narration }) {
  ...
  return createJournal({
    ...
    lines: [
      ...
      { accountId: loanReceivable.id, credit: amount, description: 'Loan receivable written off' },
    ],
  });
}
...
export async function postRecoveryPayment({
  ...
  if (!resolvedRecoveryPaymentId) {
    if (!resolvedLoanRecoveryId) {
      if (!loanApplicationId || !customerId) {
        throw new Error('Provide recoveryPaymentId, or loanRecoveryId, or (loanApplicationId + customerId) to create one');
      }
  ...
  const entry = await createJournal({
    ...
    transactionDate: paymentDate ?? new Date(),
    lines: [
      { accountId: bank.id, debit: amount, description: 'Recovery amount received' },
      { accountId: loanReceivable.id, credit: amount, description: 'Loan receivable recovered' },
    ],
  });
